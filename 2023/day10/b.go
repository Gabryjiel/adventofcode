...
func checkIfInLoop(loop []Pos, x, y int) bool {
...
func main() {
	...
	for ;; {
		...
		newPos := Pos{ x: curX, y: curY }
		loop = append(loop, newPos)
	...
	for y, row := range chars {
		shouldBeAdded := false
		for x, rowChar := range row {
			isInLoop := checkIfInLoop(loop, x, y)

			if (isInLoop) {
				if (strings.Contains(toAdd, rowChar)) {
